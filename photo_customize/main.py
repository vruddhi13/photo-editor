...
from PIL import Image, ImageTk, ImageOps, ImageEnhance, ImageFilter, ImageDraw, ImageColor
...
def add_textbox():
    ...
    text_id = canvas.create_text(canvas_width // 2, canvas_height // 2, text="Your Text Here",
                                 font=("Arial", 15), fill="white", tags="text")
...
def resize_text(event, text_id, rect_id):
    ...
    x, y, x2, y2 = canvas.coords(rect_id)
    new_size = int(abs(event.y - y))  # Calculate font size based on drag distance
    if new_size > 5:  # Minimum font size
        canvas.itemconfig(text_id, font=("Arial", new_size))
        update_rectangle(text_id, rect_id)
...
# eraser_width_slider = create_adjustment_slider_option(
#     draw_sidebar_frame,  # Parent frame
#     "Eraser",        # Label text
#     eraser_line_width,   # Command function
#     from_=1,             # Minimum line width
#     to=30                # Maximum line width
# )
...
style_dropdown = ttk.Combobox(text_sidebar_frame, values=list(text_styles.keys()), state="readonly")
style_dropdown.set("Normal")
style_dropdown.pack()

# Apply text style button
apply_style_button = tk.Button(text_sidebar_frame, text="Apply Style", command=lambda: apply_text_style(style_dropdown.get()))
apply_style_button.pack()

# Button to add new styles
add_style_button = tk.Button(text_sidebar_frame, text="Add New Style", command=add_new_style)
add_style_button.pack()

# Dropdown menu for font styles
font_dropdown = ttk.Combobox(text_sidebar_frame, values=available_fonts, state="readonly", width=30)
font_dropdown.set("Select Font Style")
font_dropdown.pack()

# Apply font style button
apply_font_button = tk.Button(text_sidebar_frame, text="Apply Font", command=lambda: apply_font_style(font_dropdown.get()))
